...
import { getComplianceReports } from 'apiSdk/compliance-reports';
...
import { DashboardListPage as ShowCaseList } from 'pages/dashboards';

function DashboardPage() {
  const { session } = useSession();

  const complianceReportStat = useSWR<PaginatedInterface<ComplianceReportInterface>>(
    () => `/compliance-reports`,
    () => getComplianceReports({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: complianceReportStat?.data?.totalCount,
      isLoading: complianceReportStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`dashboards`} passHref legacyBehavior>
                  <Button
